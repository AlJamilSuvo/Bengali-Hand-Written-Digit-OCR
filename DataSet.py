...
class DataSet:
    def __init__(self, images, labels, shape, classnames=None):
        ...
        if (classnames is None):
            self.classnames = np.array(range(self.__classes__))
        else:
    ...
    def show(self, images, real, pred=None):
        ...
        if (self.__shape__[-1] == 3):
            shape = self.__shape__
        else:
            shape = self.__shape__[:-1]

        for i, ax in enumerate(axes.flat):
            ax.imshow(images[i].reshape(shape), cmap="binary")

            if (pred is None):
                xlabel = "Real: {0}".format(self.classnames[real[i]])
            else:
                xlabel = "Real: {0}, Pred: {1}".format(self.classnames[real[i]], self.classnames[pred[i]])
    ...
    def next_batch(self, batch_size=50, offset=None):
        if (offset is None):
            dx = np.random.choice(len(self.images), size=batch_size, replace=True)
    ...
    def save(self, path):
        with gzip.open(path, "wb") as f: pickle.dump(self, f)
    ...
    def prepare_from_folder(path, height, width):
        ...
        for label, temp in enumerate(classnames):
            ...
            print("Searching in " + temp + " found " + str(len(os.listdir(folder))) + " images")
            total_image += len(os.listdir(folder))
            print
        ...
        print("------------------------------")
        print()
        print("Total Dataset Image:", total_image)
        print()
        print("------------------------------")
